...
// Singular GPE solution: Black/White Hole
...
int main()
{
    ...
    std::string B_val("B_"), N_val("_N_"), iterative_cheb_shifted_scaled_coordinate_r0("_2D_iterative_full_cheb_shifted_scaled_coordinate_r0_"), file_for_B_r0_val("r0_vs_B"), extension(".txt");
    ...
    do
    {
        ...
        {   
            ...
            do
            {
                for(int k=0;k<N-1;k++)
                {
                    F(k) = 1.-(0.25*pow(R(k)-1,2));//-0.19e2 / 0.2e1 * R(k) * R(k) + R(k) / 0.2e1 + 0.10e2;//0.1e1 - pow(R(k) / 0.2e1 - 0.1e1 / 0.2e1, 0.10e2) + 0.375e3 / 0.64e2 * pow(R(k) * R(k) - 0.1e1, 0.10e2) * pow(R(k) - 0.6e0, 0.20e2);// + 3.*pow(1-pow(R(k),2.),20.);
                    F_update(k) = F(k);
                }
                ...
                while(t<10 || ode2>1.e-6 || F_diff2>1.e-6);
            ...
            while(ode2>1.e-6 || F_diff2>1.e-6);
