...
int main()
{
    ...
    RK4(Zmax, pow(Zmax,-1./2), -(1./2)*pow(Zmax,-3./2), 0.001);
...
double ode2(double v, double rho, double drho)
{
    return (-drho / v - rho + B * B * pow(rho, -0.3e1) * pow(v, -0.2e1) + pow(rho, 0.3e1));
}
...
void RK4(double v0, double rho0, double drho0, double h)
{
    ...
    std::string B_val("B_"), h_val("_h_"), rk4("_stationary_sol_goes_to_0_at_infinity"), file_for_B_r0_val("r0_vs_B"), extension(".txt");
